...
while IFS=$'\t' read -r slug url name layer bot speed py tg suffix; do
    
    # Skip empty lines or lines starting with a comment (#)
    if [ -z "$slug" ] || [[ "$slug" == \#* ]]; then continue; fi
    
    echo "Fetching stats for $slug..."
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  -> Failed to fetch $slug. Defaulting to 0."
        stars=0
        forks=0
        # "Used by" is not exposed by GitHub's standard API natively
        used_by=0 
    fi
    ...
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
        "$link" "$layer" "$bot" "$speed" "$py" "$tg" "$stars" "$forks" "$used_by" >> "$OUTPUT_FILE"
